...
package Model::MatrizDispersaProveedorFabricante;  # ← CORREGIDO

use Model::NodoCabecera;  # ← CORREGIDO
use Model::NodoDato;      # ← CORREGIDO
...
sub _obtener_o_crear_cab_fila {
    ...
    if (!defined $self->{lista_filas}) {
        my $nueva = Model::NodoCabecera->new($fila_idx);  # ← CORREGIDO
...
sub _obtener_o_crear_cab_col {
    ...
    if (!defined $self->{lista_cols}) {
        my $nueva = Model::NodoCabecera->new($col_idx);  # ← CORREGIDO
...
sub insertar {
    ...
    my $nuevo = Model::NodoDato->new($fila_idx, $col_idx, $cantidad);  # ← CORREGIDO
